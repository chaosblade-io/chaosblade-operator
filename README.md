...
* Pod:
    ...
    * Disk: specify the directory disk occupation, disk IO read and write load, etc.
    * Memory: specify memory usage
    * Pod: kill pod, make pod stuck in ContainerCreating state by PVC mount failure, make pod stuck in Terminating state by finalizer, make pod scheduling fail by injecting unreachable affinity rules, make pod scheduling fail by adding unreachable taint to nodes
